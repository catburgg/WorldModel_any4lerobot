...
def scan_taco_sequences(taco_root: Path) -> List[Dict[str, Any]]:
    ...
    for triplet_dir in sorted(ego_rgb_root.iterdir()):
        ...
        if triplet_name not in TASK_INDICES:
            TASK_INDICES[triplet_name] = len(TASK_INDICES)
...
def build_episode_for_chunk(episode_meta: Dict[str,Any], taco_root: Path, out_data_chunk1: Path, videos_chunk_root1: Path, models_root: Path, global_index_start: int) -> Dict[str,Any]:
    ...
    episode_indices = []
    global_indices = []
    task_indices = [] # 建立三元组名和序号的一一对应

    n_frames_extracted = n_frames * FPS // 30 
    for fi in range(n_frames_extracted): # skip some frames
        fi1 = (30 // FPS) * fi # 用于取实际的帧数
        ts = fi / FPS
        timestamps.append(ts)
        frame_indices.append(fi1)
        episode_indices.append(ep_idx)
        global_indices.append(global_index_start + fi)
        next_done.append(fi == (n_frames_extracted - 1))
        ...
        rh_pose = rh[f"{fi1+1:05d}"]["hand_pose"].unsqueeze(0)
        rh_trans = rh[f"{fi1+1:05d}"]["hand_trans"].unsqueeze(0)
        lh_pose = lh[f"{fi1+1:05d}"]["hand_pose"].unsqueeze(0)
        lh_trans = lh[f"{fi1+1:05d}"]["hand_trans"].unsqueeze(0)
        ...
        extrinsic = np.load(extrinsic_p)[fi1].reshape((4,4))
        ...
        task_indices.append(TASK_INDICES[triplet_name])
    ...
    arrays["annotation.language.action_text"] = pa.array(action_texts, type=pa.string()); fields.append(pa.field("annotation.language.action_text", pa.string()))
    arrays["annotation.language.task_text"] = pa.array(task_texts, type=pa.string()); fields.append(pa.field("annotation.language.task_text", pa.string()))
    # 补充定义结束
    ...
    arrays["task_index"] = pa.array(task_indices, type=pa.int64()); fields.append(pa.field("task_index", pa.int64()))
    ...
    episode_meta_for_metajson = {
        "episode_index": ep_idx,
        "length": n_frames_extracted,
        "dataset_from_index": global_index_start,
        "dataset_to_index": global_index_start + n_frames_extracted - 1,
        "tasks": [triplet_name],
        "triplet_name": triplet_name,
